...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=-1,
                 ...
                 side_dwconv=5, before_attn_dwconv=3, pre_norm=True, auto_pad=False):#topk：用于双级路由注意力的top-k值
        super().__init__()
        qk_dim = qk_dim or dim

        # modules
        if before_attn_dwconv > 0:#before_attn_dwconv：在注意力机制（attn）之前是否使用深度可分离卷积
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 3, 1)  # (N, C, H, W) -> (N, H, W, C)将数据从 NCHW 格式转换为 NHWC 格式，以适应后续的注意力和 MLP 层，这些层通常在二维空间（HxW）上操作
